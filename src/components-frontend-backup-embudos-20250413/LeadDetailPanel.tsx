...
export function LeadDetailPanel({
  ...
  return (
    ...
    >
      ...
      {Array.isArray(lead.metadata?.messages) && lead.metadata.messages.length > 0 ? (
        <div style={{ marginBottom: 18 }}>
          ...
          <div style={{
            ...
            {lead.metadata.messages.map((msg: any, idx: number) => (
              <div key={idx} style={{ marginBottom: 14, borderLeft: "3px solid #009fe3", paddingLeft: 10 }}>
                <div style={{ fontWeight: 700, color: "#009fe3" }}>
                  {msg.subject ? msg.subject : "(Sin asunto)"}
                </div>
                <div style={{ margin: "6px 0" }}>{msg.body}</div>
                <div style={{ fontSize: 13, color: "#888" }}>
                  Fecha de origen: {msg.date_origin ? new Date(msg.date_origin).toLocaleString() : "-"}
                  <br />
                  Fecha de importación: {msg.date_import ? new Date(msg.date_import).toLocaleString() : "-"}
                </div>
              </div>
            ))}
          </div>
        ...
        lead.metadata?.body && (
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{
              ...
              <div style={{ marginBottom: 8 }}>{lead.metadata.body}</div>
              <div style={{ fontSize: 13, color: "#888" }}>
                Fecha de origen: {lead.metadata.date ? new Date(lead.metadata.date).toLocaleString() : "-"}
              </div>
            </div>
          </div>
        )
